...
class EbertReliabilityVisualizer:
    ...
    def create_reliability_distribution(self, data, save_path="reliability_distribution.png"):
        ...
        if len(non_zero_weights) > 0:
            ...
            # Add percentile thresholds if using percentiles
            if self.use_percentiles:
                percentiles = [20, 40, 60, 80]
                thresholds = np.percentile(non_zero_weights, percentiles)
                colors_percentile = ['#d73027', '#fc8d59', '#fee08b', '#a6d96a']
                for pct, thresh, color in zip(percentiles, thresholds, colors_percentile):
                    ax.axvline(thresh, color=color, linestyle=':', alpha=0.7, linewidth=2)
                    ax.text(thresh, ax.get_ylim()[1]*0.95, f'{pct}%', 
                           rotation=0, ha='center', fontsize=8, color=color, fontweight='bold')
            
            ax.set_xlabel('Weight Sum')
            ax.set_ylabel('Frequency')
            title = 'Distribution of Weight Sum (Non-Zero Values)'
            if self.use_percentiles:
                title += '\n(Colored lines show percentile thresholds)'
            ax.set_title(title)
            ax.legend()
        
        # 2. Box plot or violin plot for better distribution view
        ax = axes[0, 1]
        if len(non_zero_weights) > 0:
            # Create box plot with percentile markers
            bp = ax.boxplot([non_zero_weights], vert=False, patch_artist=True,
                           showmeans=True, meanline=True)
            bp['boxes'][0].set_facecolor('lightblue')
            bp['boxes'][0].set_alpha(0.7)
            
            # Add percentile points if using percentiles
            if self.use_percentiles:
                percentiles = [20, 40, 60, 80]
                thresholds = np.percentile(non_zero_weights, percentiles)
                colors_percentile = ['#d73027', '#fc8d59', '#fee08b', '#a6d96a']
                ax.scatter(thresholds, [1]*len(thresholds), s=100, c=colors_percentile, 
                          zorder=5, edgecolors='black', linewidth=1)
                for pct, thresh in zip(percentiles, thresholds):
                    ax.text(thresh, 1.15, f'{pct}%\n{thresh:.1f}', 
                           ha='center', fontsize=8)
            
            ax.set_xlabel('Weight Sum')
            ax.set_yticks([])
            ax.set_title('Weight Sum Distribution (Box Plot)\nwith Percentile Markers' if self.use_percentiles 
                        else 'Weight Sum Distribution (Box Plot)')
            ax.grid(True, axis='x', alpha=0.3)
        
        # 3. Category bar chart
        ax = axes[1, 0]
        category_counts = data['reliability_category'].value_counts()
        categories_ordered = ['No Data', 'Very Low', 'Low', 'Medium', 'High', 'Very High']
        ...
        title = 'Grid Points by Reliability Category'
        if self.use_percentiles:
            title += '\n(Percentile-based: each ~20% of data)'
        ax.set_title(title)
        ...
        # 4. Spatial coverage pie chart with method info
        ax = axes[1, 1]
        coverage_data = {
        ...
        if self.use_percentiles:
            title += '\n(Using Percentile-Based Categories)'
        ax.set_title(title)
