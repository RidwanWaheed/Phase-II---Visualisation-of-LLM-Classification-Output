...
class SpatialPersonalityVisualizer:
    ...
    def create_grid_visualization(self, grid_results, save_path_prefix=None):
        """
        Creates individual grid-based visualizations for each trait using projected coordinates
        for accurate proportions. Removes all axis frames, ticks, and labels for a cleaner map appearance.
        ...
        save_path_prefix : str, optional
            Prefix for saving the generated plot images. Trait name will be appended.
            
        Returns:
        --------
        dict : Dictionary of paths to the saved individual grid visualization files.
        """
        print("\nCreating individual grid-based visualizations with accurate proportions and no frames...")
        ...
        output_files = {} # To store paths of individual saved figures

        for trait in self.personality_traits:
            fig, ax = plt.subplots(1, 1, figsize=(subplot_width_inches, subplot_height_inches))
            fig.suptitle(f'Spatial Distribution of {trait}\nGrid-Based Analysis', 
            ...
            # Adjust layout for single plot
            plt.tight_layout(rect=[0, 0, 0.85, 0.95])
            
            # Add colorbar for this individual figure
            ...
            if save_path_prefix:
                trait_save_path = f"{save_path_prefix}_{trait.lower()}.png"
                plt.savefig(trait_save_path, dpi=300, bbox_inches='tight', 
                           facecolor='white', edgecolor='none')
                print(f"   Grid visualization saved: {trait_save_path}")
                output_files[trait] = trait_save_path
            
            plt.close(fig) # Close the figure to free memory
        
        return output_files
    
    def create_state_visualization(self, state_results, grid_results=None, save_path_prefix=None):
        """
        Creates individual state-level choropleth visualizations for each trait using projected coordinates
        ...
        Returns:
        --------
        dict : Dictionary of paths to the saved individual state visualization files.
        """
        ...
        print("\nCreating individual state-level visualizations with consistent and accurate proportions (no frames)...")
        ...
        for trait in self.personality_traits:
            ...
            if z_col in merged_gdf.columns:
                ...
                if trait == self.personality_traits[0]: # Only print for first trait
            ...
            if save_path_prefix:
                ...
                plt.savefig(trait_save_path, dpi=300, bbox_inches='tight', 
                           facecolor='white', edgecolor='none')
                print(f"   State visualization saved: {trait_save_path}")
                output_files[trait] = trait_save_path
...
def create_personality_visualizations(grid_results_file, original_data_file=None, 
    ...
    # Create Grid-based visualizations for each trait individually
    grid_files_output = visualizer.create_grid_visualization(grid_results, save_path_prefix=f"{output_prefix}_grid")
    output_files.update({f'grid_{trait.lower()}': path for trait, path in grid_files_output.items()})
    
    # Create State-level visualizations for each trait individually
    if state_results is not None:
        state_files_output = visualizer.create_state_visualization(state_results, grid_results, save_path_prefix=f"{output_prefix}_state")
        output_files.update({f'state_{trait.lower()}': path for trait, path in state_files_output.items()})
