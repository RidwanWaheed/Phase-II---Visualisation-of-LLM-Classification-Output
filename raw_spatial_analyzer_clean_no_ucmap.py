...
# 3. Empty vs filled cells
        ...
        ax1.set_title('Grid Cell Categories')
        
        # Add percentage labels
        total_cells = len(grid_stats)
        for bar, count in zip(bars, counts):
            percentage = count / total_cells * 100
            ax1.text(bar.get_x() + bar.get_width()/2, bar.get_height() + total_cells*0.01,
        ...
        if sum(reliability_data.values()) > 0:
            wedges, texts, autotexts = ax2.pie(reliability_data.values(), 
                                              labels=reliability_data.keys(),
                                              autopct='%1.1f%%',
                                              colors=['lightgray', 'lightcoral', 'lightyellow', 'lightgreen'])
            ax2.set_title('Data Reliability Distribution')
        else:
    ...
    def _create_data_quality_maps(self, grid_stats, save_path):
        ...
        for zone, color in reliability_colors.items():
            ...
            if len(subset) > 0:
                subset.plot(ax=ax1, color=color, alpha=0.8, edgecolor='black', linewidth=0.2)
        
        if self.boundary_gdf is not None:
            self.boundary_gdf.boundary.plot(ax=ax1, color='black', linewidth=1)
        
        ax1.set_title('Reliability Zones')
        ax1.set_aspect('equal', adjustable='box')
        ...
        if len(empty_cells) > 0:
            empty_cells.plot(ax=ax2, color='lightgray', alpha=0.8, label='No data', edgecolor='black', linewidth=0.1)
        if len(filled_cells) > 0:
            filled_cells.plot(ax=ax2, color='blue', alpha=0.6, label='Has data', edgecolor='black', linewidth=0.1)
        
        if self.boundary_gdf is not None:
            self.boundary_gdf.boundary.plot(ax=ax2, color='black', linewidth=1)
        
        ax2.set_title('Data Coverage')
        ax2.set_aspect('equal', adjustable='box')
        ax2.legend()
